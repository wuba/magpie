...
/*
* pub环境下读取.pub-cache/hosted里源码文件夹路径
* 源码环境下 读取文件路径
* */
String userRootPath() {
  if (cliEnvironment() == CliEnvironment.debug) {
    return fs.path.join(Cache.mpcliRoot);
  }
  String fileRoot;
  if (platform.operatingSystem == 'windows') {
    fileRoot = fs.path.join(userHomePath(), 'pub', 'cache', 'hosted',
        'pub.flutter-io.cn', 'magpie_cli-$cliVersion');
    if (!fs.directory(fileRoot).existsSync()) {
      fileRoot = fs.path.join(userHomePath(), 'pub', 'cache', 'hosted',
          'pub.dartlang.org', 'magpie_cli-$cliVersion');
    }
  } else {
    fileRoot = fs.path.join(userHomePath(), '.pub-cache', 'hosted',
        'pub.flutter-io.cn', 'magpie_cli-$cliVersion');
...
String homeResourcePath() {
  return fs.path.join(userHomePath(), '.mgpcli');
}

FileSystem localFileSystem = const LocalFileSystem();
...
String currentFolderPath() {
  return localFileSystem.currentDirectory.path;
}

void initProjectRootPath() {
  // set flutter path
  ...
  if (scriptRoot.contains(midPath)) {
    Cache.mpcliRoot = scriptRoot.substring(0, scriptRoot.lastIndexOf(midPath));
  } else {
    print('init mgpcli root path error ...');
  }
}
