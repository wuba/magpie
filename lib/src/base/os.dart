...
OperatingSystemUtils get os =>
    context.get<OperatingSystemUtils>() ?? OperatingSystemUtils();
...
abstract class OperatingSystemUtils {
  ...
  int launchWorkflow();
  void launchWeb(var url);
  void killProcessByName(var name);
}
...
class _PosixUtils extends OperatingSystemUtils {
  ...
  void chmod(FileSystemEntity entity, String mode) {
    try {
      final ProcessResult result =
          processManager.runSync(<String>['chmod', '-R', mode, entity.path]);
  ...
  @override
  int launchWorkflow() {
    var commandPath = which('mgpcli').path;
    final int code = processManager
        .runSync(<String>['open', '-a', 'Terminal.app', commandPath]).exitCode;
    return code;
  }

  @override
  void launchWeb(var url) {
    processManager.runSync(<String>['open', url]);
  }

  @override
  void killProcessByName(var name) async {
    final Process process = await runCommand(<String>['ps', '-ef']);
    process.stdout.transform(utf8.decoder).listen((data) {
      List<String> psmap = data.split('\n');
      for (String psinfo in psmap) {
        if (psinfo.contains(name)) {
          // kill pid
          List<String> ff = psinfo.split(' ');
          String pid = ff[3];
...
class _WindowsUtils extends OperatingSystemUtils {
  ...
  int launchWorkflow() {
    ...
    final ProcessResult result = processManager.runSync(
        <String>['start', 'magpie workflow', commandPath],
        runInShell: true);
  ...
  @override
  void launchWeb(var url) {
    processManager.runSync(<String>['start', '', url], runInShell: true);
  }
