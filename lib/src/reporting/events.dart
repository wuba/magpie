...
class DoctorResultEvent extends UsageEvent {
  ...
  }) : super('doctorResult.${validator.runtimeType}', result.typeStr);
...
class BuildEvent extends UsageEvent {
  BuildEvent(
    ...
    this.settings,
            ...
            'build' +
                (MpcliCommand.current == null
                    ? ''
                    : '-${MpcliCommand.current.name}'),
            parameter);
  ...
  void send() {
    final Map<String, String> parameters =
        _useCdKeys(<CustomDimensions, String>{
      if (command != null) CustomDimensions.buildEventCommand: command,
      if (settings != null) CustomDimensions.buildEventSettings: settings,
    });
    flutterUsage.sendEvent(category, parameter, parameters: parameters);
  }
}
...
class CommandResultEvent extends UsageEvent {
  CommandResultEvent(String commandPath, MpcliCommandResult result)
