...
class Cache {
  ...
  static Future<void> lock() async {
    ...
    assert(_lock == null);
    final File lockFile =
        fs.file(fs.path.join(mpcliRoot, 'bin', 'cache', 'lockfile'));
    try {
    ...
    while (!locked) {
      try {
        ...
        if (!printed) {
          ...
          printStatus(
              'Waiting for another mgpcli command to release the startup lock...');
          printed = true;
        }
  ...
  static void checkLockAcquired() {
    if (_lockEnabled &&
      ...
      throw StateError(
        'The current process does not own the lock for the cache directory. This is a bug in mgpcli CLI tools.',
      );
    }
  }
...
abstract class EngineCachedArtifact extends CachedArtifact {
  ...
  Future<void> updateInner() async {
    ...
    final File licenseSource =
        fs.file(fs.path.join(Cache.mpcliRoot, 'LICENSE'));
    for (String licenseDir in getLicenseDirs()) {
