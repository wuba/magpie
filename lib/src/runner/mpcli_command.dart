...
/// [MpcliCommand]s' subclasses' [MpcliCommand.runCommand] can optionally
/// provide a [MpcliCommandResult] to furnish additional information for
/// analytics.
class MpcliCommandResult {
  const MpcliCommandResult(
    this.exitStatus, {
  ...
  ///
  /// [MpcliCommand] will automatically measure and report the command's
...
abstract class MpcliCommand extends Command<void> {
  /// The currently executing command (or sub-command).
  ///
  /// Will be `null` until the top-most command has begun execution.
  static MpcliCommand get current => context.get<MpcliCommand>();
  ...
  @override
  CommandRunner get runner => runner;
  ...
  Future<String> get usagePath async {
    if (parent is MpcliCommand) {
      final MpcliCommand commandParent = parent;
  ...
  Future<void> run() {
    ...
    return context.run<void>(
      name: 'command',
      overrides: <Type, Generator>{MpcliCommand: () => this},
      body: () async {
        if (flutterUsage.isFirstRun) {
          flutterUsage.printWelcome();
        }
        final String commandPath = await usagePath;
        MpcliCommandResult commandResult;
        try {
          commandResult = await verifyThenRunCommand(commandPath);
        } on ToolExit {
          commandResult = const MpcliCommandResult(ExitStatus.fail);
          rethrow;
        } finally {
  ...
  void _sendPostUsage(String commandPath, MpcliCommandResult commandResult,
  ...
  @mustCallSuper
  Future<MpcliCommandResult> verifyThenRunCommand(String commandPath) async {
    await validateCommand();

    // check mgpcli run environment
    String magpie = fs.path.join(currentFolderPath(), '.magpie');
    printStatus('');
    if (!fs.file(magpie).existsSync() &&
        commandPath != 'create' &&
        commandPath != 'service' &&
        commandPath != 'devices') {
      printStatus(userMessages.invalidCLIPathMessage(currentFolderPath()));
  ...
  Future<MpcliCommandResult> runCommand();
...
mixin DeviceBasedDevelopmentArtifacts on MpcliCommand {
  @override
  Future<Set<DevelopmentArtifact>> get requiredArtifacts async {
...
mixin TargetPlatformBasedDevelopmentArtifacts on MpcliCommand {
  @override
  Future<Set<DevelopmentArtifact>> get requiredArtifacts async {
...
abstract class FastMpcliCommand extends MpcliCommand {
  @override
  Future<void> run() {
    return context.run<void>(
      name: 'command',
