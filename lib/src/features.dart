...
class FeatureFlags {
  ...
  bool get isLinuxEnabled => _isEnabled(mpcliLinuxDesktopFeature);

  /// Whether flutter desktop for macOS is enabled.
  bool get isMacOSEnabled => _isEnabled(mpcliMacOSDesktopFeature);

  /// Whether flutter web is enabled.
  bool get isWebEnabled => _isEnabled(mpcliWebFeature);

  /// Whether flutter desktop for Windows is enabled.
  bool get isWindowsEnabled => _isEnabled(mpcliWindowsDesktopFeature);

  /// Whether plugins are built as AARs in app projects.
  bool get isPluginAsAarEnabled => _isEnabled(mpcliBuildPluginAsAarFeature);
  ...
  static bool _isEnabled(Feature feature) {
    ...
    if (feature.configSetting != null) {
      final bool configOverride =
          Config.instance.getValue(feature.configSetting);
      if (configOverride != null) {
        isEnabled = configOverride;
      }
    }
    if (feature.environmentOverride != null) {
      if (platform.environment[feature.environmentOverride]?.toLowerCase() ==
          'true') {
        isEnabled = true;
      }
    }
...
const List<Feature> allFeatures = <Feature>[
  mpcliWebFeature,
  mpcliLinuxDesktopFeature,
  mpcliMacOSDesktopFeature,
  mpcliWindowsDesktopFeature,
  mpcliBuildPluginAsAarFeature,
];

/// The [Feature] for mgpcli web.
const Feature mpcliWebFeature = Feature(
  name: 'Mpcli for web',
...
const Feature mpcliMacOSDesktopFeature = Feature(
  name: 'Mpcli for desktop on macOS',
...
const Feature mpcliLinuxDesktopFeature = Feature(
  name: 'Mpcli for desktop on Linux',
...
const Feature mpcliWindowsDesktopFeature = Feature(
  name: 'Mpcli for desktop on Windows',
...
const Feature mpcliBuildPluginAsAarFeature = Feature(
