...
Future<void> pubGet({
  ...
  if (!checkLastModified ||
    ...
    final bool verbose = MpcliCommand.current != null &&
        MpcliCommand.current.globalResults['verbose'];
  ...
  if (dotPackages.lastModifiedSync().isBefore(pubSpecYaml.lastModifiedSync())) {
    throwToolExit(
        '$directory: pub did not update .packages file (pubspec.yaml timestamp: ${pubSpecYaml.lastModifiedSync()}; .packages timestamp: ${dotPackages.lastModifiedSync()}).');
  }
}
