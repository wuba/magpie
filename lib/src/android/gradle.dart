...
Future<File> getGradleAppOut(AndroidProject androidProject) async {
  switch (getFlutterPluginVersion(androidProject)) {
    ...
    case FlutterPluginVersion.v2:
      return fs
          .file((await _gradleAppProject()).apkDirectory.childFile('app.apk'));
  }
  return null;
}
...
void createSettingsAarGradle(Directory androidDirectory) {
  final File newSettingsFile =
      androidDirectory.childFile('settings_aar.gradle');
  if (newSettingsFile.existsSync()) {
    return;
  }
  final File currentSettingsFile =
  ...
  final Status status = logger.startProgress(
      '✏️  Creating `$newSettingsRelativeFile`...',
      timeout: timeoutConfiguration.fastOperation);

  final String flutterRoot = fs.path.absolute(Cache.flutterRoot);
  final File deprecatedFile = fs.file(fs.path.join(flutterRoot, 'packages',
      'flutter_tools', 'gradle', 'deprecated_settings.gradle'));
  assert(deprecatedFile.existsSync());
  final String settingsAarContent = fs
      .file(fs.path.join(flutterRoot, 'packages', 'flutter_tools', 'gradle',
  ...
  final List<String> existingVariants =
      deprecatedFile.readAsStringSync().split(';EOF');
  ...
  if (!exactMatch) {
    ...
    printError(
        '*******************************************************************************************');
    printError(
        'Flutter tried to create the file `$newSettingsRelativeFile`, but failed.');
...
Future<GradleProject> _readGradleProject({bool isLibrary = false}) async {
  ...
  try {
    ...
    project = GradleProject(
        ...
        <String>[],
        fs.path.join(
            flutterProject.android.hostAppGradleRoot.path, 'app', 'build'));
  }
  status.stop();
  return project;
}
...
final RegExp _androidPluginRegExp =
    RegExp('com\.android\.tools\.build\:gradle\:(\\d+\.\\d+\.\\d+\)');
  ...
  switch (getFlutterPluginVersion(project.android)) {
    ...
    case FlutterPluginVersion.v2:
      return _buildGradleProjectV2(
          project, gradle, androidBuildInfo, target, isBuildingBundle);
  }
}
...
Future<void> buildGradleAar({
  ...
  flutterUsage.sendTiming(
      'build', 'gradle-aar', Duration(milliseconds: sw.elapsedMilliseconds));

  if (exitCode != 0) {
    throwToolExit('Gradle task $aarTask failed with exit code $exitCode',
        exitCode: exitCode);
  }

  final Directory repoDirectory = gradleProject.repoDirectory;
  if (!repoDirectory.existsSync()) {
    throwToolExit('Gradle task $aarTask failed to produce $repoDirectory',
        exitCode: exitCode);
  }
  printStatus('Built ${fs.path.relative(repoDirectory.path)}.',
...
Future<void> _buildGradleProjectV1(
  ...
  status.stop();
  flutterUsage.sendTiming(
      'build', 'gradle-v1', Duration(milliseconds: sw.elapsedMilliseconds));
...
String _calculateSha(File file) {
  ...
  flutterUsage.sendTiming(
      'build', 'apk-sha-read', Duration(milliseconds: sw.elapsedMilliseconds));
  sw.reset();
  final String sha = _hex(sha1.convert(bytes).bytes);
  printTrace('calculateSha: computing sha took ${sw.elapsedMilliseconds}us');
  flutterUsage.sendTiming(
      'build', 'apk-sha-calc', Duration(milliseconds: sw.elapsedMilliseconds));
  return sha;
}
...
Future<void> _buildGradleProjectV2(
  ...
  if (androidBuildInfo.splitPerAbi) command.add('-Psplit-per-abi=true');
  if (androidBuildInfo.targetArchs.isNotEmpty) {
    final String targetPlatforms = androidBuildInfo.targetArchs
        .map(getPlatformNameForAndroidArch)
        .join(',');
  ...
  try {
    exitCode = await runCommandAndStreamOutput(
      ...
      mapFunction: (String line) {
        ...
        if (logger.isVerbose) {
          return line;
        } else if (isAndroidXPluginWarning ||
            !ndkMessageFilter.hasMatch(line)) {
          return null;
        }
  ...
  flutterUsage.sendTiming(
      'build', 'gradle-v2', Duration(milliseconds: sw.elapsedMilliseconds));
...
Iterable<File> findApkFiles(
    GradleProject project, AndroidBuildInfo androidBuildInfo) {
  final Iterable<String> apkFileNames = project.apkFilesFor(androidBuildInfo);
  if (apkFileNames.isEmpty) return const <File>[];

  return apkFileNames.expand<File>((String apkFileName) {
    File apkFile = project.apkDirectory.childFile(apkFileName);
    if (apkFile.existsSync()) return <File>[apkFile];
    final BuildInfo buildInfo = androidBuildInfo.buildInfo;
    final String modeName = camelCase(buildInfo.modeName);
    apkFile =
        project.apkDirectory.childDirectory(modeName).childFile(apkFileName);
    if (apkFile.existsSync()) return <File>[apkFile];
...
class GradleProject {
  ...
  Iterable<String> apkFilesFor(AndroidBuildInfo androidBuildInfo) {
    ...
    if (buildType == null || productFlavor == null) return const <String>[];
